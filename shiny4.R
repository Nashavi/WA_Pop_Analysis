...
shinyApp(
  ui = fluidPage(
    leafletOutput('WaMap',width="100%",height="750px"),
  ...
  server = function(input, output) {
    ...
    b<-a %>% dplyr::group_by(full_add) %>% dplyr::mutate(s_values=scale(value,center=T,scale=F))
    ...
    ggplot(b[b$full_add %in% m2_counties$full_add,])+geom_line(aes(x=yr,y=s_values,color=factor(JURISDICTION)))+facet_wrap(~COUNTY,scales = "free_y")+ scale_colour_discrete(guide = FALSE) + theme_bw() +ylab("Increase in residents\n(Centered around mean)") + xlab("Year")+theme(axis.text.x=element_text(angle=50, vjust=0.5)) +ggtitle("Population growth in WA counties\n(Considering only cities that have changed by atleast 5000 during the period)")
    
    pal<-colorBin(palette=rainbow(9), a$g2, bins = c(0,0.25,0.50,0.75,1,2,3,4,5,7,10,12,15), na.color = NA, pretty=TRUE, alpha = TRUE)
